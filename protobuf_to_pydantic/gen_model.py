...
def replace_file_name_to_class_name(filename: str) -> str:
    ...
    prefix: str = "".join(
        [str(i).title() for i in Path(filename.split(".")[0]).joinpath().parts]
    )
...
class CodeRefModel(object):
    ...
    def from_model(cls, model: Type[BaseModel]) -> "CodeRefModel":
        ...
        raise ValueError(
            "Not found CodeRefModel, please set `enable_code_ref_gen==True` in gen_model func or class"
        )
...
class M2P(object):
    def __init__(
        ...
        if proto_file_name.endswith("empty.proto") or parse_msg_desc_method == "ignore":
            ...
            isinstance(parse_msg_desc_method, str)
            and Path(parse_msg_desc_method).exists()
        ):
            ...
            global_message_option_dict = get_message_option_dict_from_pyi_file(
                pyi_file_name, comment_prefix
            )
        elif parse_msg_desc_method == "PGV":
            # get field dict from pgv
            global_message_option_dict = get_message_option_dict_from_message_with_pgv(
                message=msg
            )  # type: ignore
            ...
            global_message_option_dict = get_message_option_dict_from_message_with_p2p(
                message=msg
            )  # type: ignore
        ...
        self._message_default_factory_dict_by_type_name: Dict[str, Any] = (
            message_default_factory_dict_by_type_name
            or constant.message_name_default_factory_dict
        )
    ...
    def _get_field_info_dict_by_full_name(
        ...
        if message_name not in self._message_option_dict:
            return None
        message_option_dict: "MessageOptionTypedDict" = self._message_option_dict[
            message_name
        ]
    ...
    def _is_type_a_mapping(self, message_type: Descriptor) -> bool:
        return message_type.name.endswith(
            "Entry"
        ) and message_type.fields_by_name.keys() == ["key", "value"]

    def _one_of_handle(
        self, descriptor: Descriptor
    ) -> Tuple[Dict[str, "UseOneOfTypedDict"], Dict[str, Any]]:
        message_option_dict: "MessageOptionTypedDict" = self._message_option_dict.get(
            descriptor.name, {"message": {}, "one_of": {}, "nested": {}, "metadata": {}}
        )
        ignore_parse_rule = message_option_dict.get("metadata", {}).get(
            "ignored", False
        )
        one_of_desc_dict = {}
        ...
        for one_of in descriptor.oneofs:
            ...
            for found_column_name in [
                field_full_name,
                ".".join(field_full_name.split(".")[1:]),
            ]:
                if found_column_name not in one_of_desc_dict:
                    continue
                # only PGV or P2P support
                one_of_dict[field_full_name]["required"] = one_of_desc_dict[
                    found_column_name
                ].get("required", False)
                optional_fields = one_of_desc_dict[found_column_name].get(
                    "optional_fields", set()
                )
                if optional_fields:
                    for field_name in optional_fields:
                        optional_dict[descriptor.full_name + "." + field_name] = {
    ...
    def get_nested_message_dict_by_message(
        self, descriptor: Descriptor
    ) -> Dict[str, Type[Union[BaseModel, IntEnum]]]:
        # nested support
        nested_message_dict: Dict[str, Type[Union[BaseModel, IntEnum]]] = {}
        for message in descriptor.nested_types:
            ...
            nested_type: Any = self._parse_msg_to_pydantic_model(
                descriptor=message, root_descriptor=descriptor
            )
    ...
    # field  handler   #
    ####################
    def _protobuf_field_type_is_type_message_handler(
        self, field_dataclass: FieldDataClass
    ) -> None:
        ...
        if protobuf_field.message_type.name in self._message_type_dict_by_type_name:
            ...
            field_dataclass.field_type = self._message_type_dict_by_type_name[
                protobuf_field.message_type.name
            ]
            if (
                protobuf_field.message_type.name
                in self._message_default_factory_dict_by_type_name
            ):
                # Default factory has a higher priority
                field_dataclass.field_default_factory = (
                    self._message_default_factory_dict_by_type_name[
                        protobuf_field.message_type.name
                    ]
                )
            ...
            for k_v_field in protobuf_field.message_type.fields:
                if not k_v_field.message_type:
                    k_v_type: Any = constant.protobuf_desc_python_type_dict[
                        k_v_field.type
                    ]
                elif (
                    k_v_field.message_type.name in self._message_type_dict_by_type_name
                ):
                    k_v_type = self._message_type_dict_by_type_name[
                        k_v_field.message_type.name
                    ]
                else:
                    k_v_type = self._parse_msg_to_pydantic_model(
                        descriptor=k_v_field.message_type,
                        root_descriptor=protobuf_field.message_type,
                    )
            ...
            module_name = (
                protobuf_field.message_type.file.name.split(".")[0].replace("/", ".")
                + "_pb2"
            )
            type_factory = getattr(
                importlib.import_module(module_name), protobuf_field.message_type.name
            )
            ...
            field_info_dict: Union[FieldInfoTypedDict, dict] = (
                self._get_field_info_dict_by_full_name(
                    field_dataclass.protobuf_field.full_name
                )
                or {}
            )
            ...
            if full_name in field_dataclass.nested_message_dict:
                _cache_full_name = (
                    full_name + SKIP_RULE_MESSAGE_SUFFIX
                ...
                if _cache_full_name not in field_dataclass.nested_message_dict:
                    ...
                    nested_message = [
                        i
                        for i in field_dataclass.descriptor.nested_types
                        if i.full_name == full_name
                    ][0]
                    nested_type: Any = self._parse_msg_to_pydantic_model(
                        descriptor=nested_message,
                        class_name=protobuf_field.message_type.name
                        + SKIP_RULE_MESSAGE_SUFFIX,
                        skip_validate_rule=skip_validate_rule,
                    )
                    field_dataclass.nested_message_dict[_cache_full_name] = nested_type
                    field_dataclass.field_type = field_dataclass.nested_message_dict[
                        _cache_full_name
                    ]
                else:
                    field_dataclass.field_type = field_dataclass.nested_message_dict[
                ...
                is_same_pkg: bool = (
                    field_dataclass.descriptor.file.name
                    == protobuf_field.message_type.file.name
                )
                if not is_same_pkg:
                    ...
                    if (
                        skip_validate_rule
                        or protobuf_field.message_type.full_name
                        != field_dataclass.descriptor.full_name
                    ):
                        try:
                            field_dataclass.field_type = (
                                self._parse_msg_to_pydantic_model(
                                    descriptor=protobuf_field.message_type,
                                    class_name=_class_name
                                    if not skip_validate_rule
                                    else _class_name + "OnlyUseSkipRule",
                                    skip_validate_rule=skip_validate_rule,
                                )
                            )
    ...
    def _protobuf_field_type_is_type_enum_handler(
        self, field_dataclass: FieldDataClass
    ) -> None:
        ...
        if protobuf_field.enum_type.full_name in field_dataclass.nested_message_dict:
            field_dataclass.field_type = field_dataclass.nested_message_dict[
                protobuf_field.enum_type.full_name
            ]
            # Facilitate the analysis of `gen code`
            setattr(field_dataclass.field_type, "_is_nested", True)
        else:
            enum_class_dict = {
                v.name: v.number for v in protobuf_field.enum_type.values
            }
            _class_name = protobuf_field.enum_type.name
            _class_doc = ""
            if (
                field_dataclass.descriptor.file.name
                != protobuf_field.enum_type.file.name
            ):
                _class_name = (
                    replace_file_name_to_class_name(protobuf_field.enum_type.file.name)
                    + _class_name
                )
                _class_doc = (
                    "Note: The current class does not belong to the package\n"
    ...
    def _protobuf_field_lable_is_label_repeated_handler(
        self, field_dataclass: FieldDataClass
    ) -> None:
        # support google.protobuf.array
        protobuf_field = field_dataclass.protobuf_field
        if not (
            protobuf_field.message_type
            and self._is_type_a_mapping(protobuf_field.message_type)
        ):
    ...
    def _gen_field_info(
        self, field_dataclass: FieldDataClass, skip_validate_rule: bool
    ) -> Optional[FieldInfo]:
        field_class = self._default_field
        field_info_dict = self._get_field_info_dict_by_full_name(
            field_dataclass.protobuf_field.full_name
        )
        ...
        if field_info_dict is not None and not skip_validate_rule:
            if self._parse_msg_desc_method != "PGV":
                ...
                field_info_dict = self._comment_template.handle_template_var(
                    field_info_dict
                )
            if not (
                self._parse_msg_desc_method is None
                or self._parse_msg_desc_method == "PGV"
            ):
            ...
            field_info_dict: FieldInfoTypedDict = FieldInfoParamModel(
                **field_info_dict
            ).dict()  # type: ignore
    ...
    def _parse_msg_to_pydantic_model(
        ...
        *,
        ...
        is_same_pkg = (
            descriptor.file.name == root_descriptor.file.name
            if root_descriptor
            else True
        )
        ...
        if not is_same_pkg:
            class_name = (
                replace_file_name_to_class_name(descriptor.file.name) + class_name
            )
        ...
        if message_key in self._creat_cache:
            if self._creat_cache[message_key] is None:
                raise WaitingToCompleteException(
                    f"The model:{message_key} is being generated"
                )
        ...
        for protobuf_field in descriptor.fields:
            field_dataclass = FieldDataClass(
                ...
                field_type=constant.protobuf_desc_python_type_dict.get(
                    protobuf_field.type, None
                ),
                field_type_name=protobuf_common_type_dict.get(
                    protobuf_field.type, None
                ),  # type: ignore
            ...
            if one_of_dict:
                model_config_dict = self._get_pydantic_base(
            ...
            if (
                ...
                and not _pydantic_adapter.get_model_config_value(
        ...
        CodeRefModel.set_to_model(
            ...
            nested_message_dict={
                k: v
                for k, v in nested_message_dict.items()
                if getattr(v, "_is_nested", False)
            },
            validators=validators,
        )
