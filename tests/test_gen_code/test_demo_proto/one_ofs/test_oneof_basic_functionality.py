...
class TestOneofBasicFunctionality:
    ...
    def test_oneof_field_switching_immutable(self):
        ...
        try:
            ...
            assert "Multiple fields" in str(e) or "multiple oneof fields" in str(e).lower()
    ...
    def test_oneof_dict_representation(self):
        ...
        print(f"Model dump (exclude_none): {data_clean}")
        
        # Test with ReportData
        geo = GeoLocation(latitude=37.7749, longitude=-122.4194)
        report = ReportData(location_value=geo)
        report_dict = report.model_dump()
        print(f"ReportData dump: {report_dict}")
    ...
    def test_oneof_validation_with_invalid_discriminator(self):
        ...
        # Some might auto-correct, others might raise an error
        try:
            ...
            # If this succeeds, check what actually got set
            print(f"Discriminator mismatch resulted in: {msg.a.a_case}")
        except (ValidationError, ValueError) as e:
            # This is the expected behavior
            print(f"Discriminator mismatch correctly rejected: {e}")

    def test_oneof_none_value_behavior(self):
        """Test how oneofs handle None values."""
        # Test 1: Try to set a field to None explicitly
        try:
            msg = OptionalMessage(x=None)
            # If this succeeds, check the state
            print(f"Setting x=None resulted in: {msg}")
            if hasattr(msg, 'a'):
                print(f"Oneof case: {getattr(msg.a, 'a_case', 'no case')}")
        except (ValidationError, TypeError, ValueError) as e:
            print(f"Setting x=None raised: {type(e).__name__}: {e}")
        
        # Test 2: Create without any oneof field
        try:
            msg2 = OptionalMessage(name="test")
            print(f"Creating without oneof resulted in: {msg2}")
            if hasattr(msg2, 'a'):
                print(f"Oneof case: {getattr(msg2.a, 'a_case', 'no case')}")
        except (ValidationError, ValueError) as e:
            print(f"Creating without oneof raised: {type(e).__name__}: {e}")
        
        # Test 3: Empty dict for oneof
        try:
            msg3 = OptionalMessage.model_validate({'name': 'test', 'a': {}})
            print(f"Empty oneof dict resulted in: {msg3}")
        except (ValidationError, ValueError) as e:
            print(f"Empty oneof dict raised: {type(e).__name__}: {e}")