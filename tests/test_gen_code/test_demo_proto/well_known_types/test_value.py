...
class TestValue(RoundTripTestBase):
    ...
    def test_value_number(self):
        ...
        for number in test_numbers:
            proto_msg.Clear()
            proto_msg.single_value.number_value = number
    ...
    def test_value_string(self):
        ...
        for string in test_strings:
            proto_msg.Clear()
            proto_msg.single_value.string_value = string
    ...
    def test_value_bool(self):
        ...
        # Test false
        proto_msg.Clear()
        proto_msg.single_value.bool_value = False
        self.verify_roundtrip(proto_msg, value_demo_p2p.ValueDemo)
    ...
    def test_value_struct(self):
        ...
        # Simple struct
        proto_msg.single_value.struct_value.fields["name"].string_value = "John"
        proto_msg.single_value.struct_value.fields["age"].number_value = 30
        proto_msg.single_value.struct_value.fields["active"].bool_value = True
        proto_msg.single_value.struct_value.fields["email"].null_value = 0
    ...
    def test_value_nested_struct(self):
        ...
        root = proto_msg.single_value.struct_value
    ...
    def test_value_empty_collections(self):
        ...
        # Empty struct
        proto_msg.single_value.struct_value.CopyFrom(
            proto_msg.single_value.struct_value
        )
        self.verify_roundtrip(proto_msg, value_demo_p2p.ValueDemo)
        
        # Empty list
        proto_msg.Clear()
        proto_msg.single_value.list_value.CopyFrom(
            proto_msg.single_value.list_value
        )
        self.verify_roundtrip(proto_msg, value_demo_p2p.ValueDemo)
    ...
    def test_value_map(self):
        ...
        # Add struct to map
        proto_msg.value_map["object"].struct_value.fields["nested"].string_value = "value"
    ...
    def test_value_from_pydantic_dict(self):
        ...
        pydantic_model = value_demo_p2p.ValueDemo(
            single_value={
                "name": "Test",
                "count": 42,
                "active": True,
        ...
        proto_msg = self.json_to_protobuf(json_str, value_demo_pb2.ValueDemo)
        
        # Verify structure
        assert proto_msg.single_value.struct_value.fields["name"].string_value == "Test"
        assert proto_msg.single_value.struct_value.fields["count"].number_value == 42
        assert proto_msg.single_value.struct_value.fields["active"].bool_value == True
        
        # Verify nested list
        tags_list = proto_msg.single_value.struct_value.fields["tags"].list_value
        assert len(tags_list.values) == 2
        assert tags_list.values[0].string_value == "python"
        
        # Verify nested struct
        metadata = proto_msg.single_value.struct_value.fields["metadata"].struct_value
    ...
    def test_value_json_representation(self):
        ...
        # Set a struct value
        proto_msg.single_value.struct_value.fields["foo"].string_value = "bar"
        proto_msg.single_value.struct_value.fields["num"].number_value = 42
        
        # Convert to JSON
        json_str = self.protobuf_to_json(proto_msg)
        
        # Should serialize as plain JSON object, not wrapped
        assert '"singleValue": {"foo": "bar", "num": 42}' in json_str

    def test_value_special_float_values(self):
        """Test Value with special float values."""
        proto_msg = value_demo_pb2.ValueDemo()
        
        # Note: JSON format for protobuf converts special float values to strings
        special_values = [
            float('inf'),
            float('-inf'),
            float('nan'),
        ...
        for value in special_values:
            proto_msg.Clear()
            proto_msg.single_value.number_value = value
            
            # For NaN, we need special handling since NaN != NaN
            if value != value:  # Check for NaN
                json_str = self.protobuf_to_json(proto_msg)
                assert '"singleValue": "NaN"' in json_str
            else:
                self.verify_roundtrip(proto_msg, value_demo_p2p.ValueDemo)