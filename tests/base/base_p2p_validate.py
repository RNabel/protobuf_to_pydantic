...
class BaseTestP2pModelValidator:
    ...
    replace_message_fn: Callable = staticmethod(
        lambda model_class, **kwargs: model_class
    )  # type: ignore

    def _check_message_validate(
        self, message_class: Any, normal_dict: dict, error_map_dict: dict
    ) -> None:
        ...
        for key, value in error_map_dict.items():
            ...
            with pytest.raises(ValidationError):
                self.replace_message_fn(message_class, local_dict=local_dict)(
                    **error_normal_dict
                )
    ...
    def test_number_model_in_validator(self) -> None:
        for model_class in self.number_model_class_list:
            for i in [1, 2, 3]:
                self.replace_message_fn(
                    model_class, local_dict=local_dict, template=CustomCommentTemplate
                )(in_test=i, required_test=1.0)
            for i in [0, 4]:
                with pytest.raises(ValidationError):
                    self.replace_message_fn(
                        model_class,
            ...
            for i in [1, 2, 3]:
                with pytest.raises(ValidationError):
                    self.replace_message_fn(
                        ...
                        local_dict=local_dict,
                        template=CustomCommentTemplate,
                    )(not_in_test=i, required_test=1.0)
            for i in [0, 4]:
                self.replace_message_fn(
                    model_class, local_dict=local_dict, template=CustomCommentTemplate
                )(not_in_test=i, required_test=1.0)
    ...
    def test_number_model_miss_multiple_of_validator(self) -> None:
        for model_class in self.number_model_class_list:
            self.replace_message_fn(
                model_class, local_dict=local_dict, template=CustomCommentTemplate
            )(multiple_of_test=6.0, required_test=1.0)
            with pytest.raises(ValidationError):
                self.replace_message_fn(
                    model_class, local_dict=local_dict, template=CustomCommentTemplate
                )(multiple_of_test=7.0, required_test=1.0)
